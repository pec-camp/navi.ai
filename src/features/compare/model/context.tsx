...
function compareReducer(state: CompareState, action: CompareAction): CompareState {
  switch (action.type) {
    case "ADD_ITEM": {
      ...
      const exists = state.items.some(item => item.tool.id === action.payload.id);
      if (exists) {
        return state;
      }
...
const CompareContext = createContext<(CompareState & CompareActions) | undefined>(undefined);
...
export function CompareProvider({ children }: CompareProviderProps) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      localStorage.setItem(COMPARE_CONFIG.STORAGE_KEY, JSON.stringify(dataToStore));
    } catch (error) {
