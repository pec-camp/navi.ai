...
async function scanDirectory(dir: string, segments: string[] = []): Promise<RouteInfo[]> {
      ...
      try {
        ...
        for (const parallelDir of parallelDirs) {
          ...
          try {
            ...
            const interceptingDirs = parallelContents.filter(name => name.startsWith('(.'));
...
function generateTypeScript(routes: RouteInfo[]): string {
  const constants = routes
    ...
    .map(route => {
      if (route.hasParams && route.params) {
        const paramTypes = route.params.map(param => `${param}: string`).join(', ');
  ...
  const routeList = routes.map(route => `  ${route.constantName}`).join(',\n');
